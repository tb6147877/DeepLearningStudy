...
def predict(w, b, X):
    ...
    for i in range(A.shape[1]):
        ...
        Y_prediction[0, i] = 1 if A[0, i] > 0.5 else 0  # 三元表达式
...
def model(X_train, Y_train, X_test, Y_test, num_iterations=2000, learning_rate=0.5, print_cost=False):
    ...
    w, b = initialize_with_zeros(X_train.shape[0]) # init params

    # Gradient descent (≈ 1 line of code)
    parameters, grads, costs = optimize(w, b, X_train, Y_train, num_iterations, learning_rate, print_cost) # train model
    ...
    # Predict test/train set examples (≈ 2 lines of code)
    Y_prediction_test = predict(w, b, X_test) #use model to pridict
